  ...
  return (
    <>
      ...
      <div className="flex justify-between w-full h-auto m-3 p-2 text-2xl">
        <span className="text-left text-maroon">Expiring Soon</span>
        <button onClick={() => navigate('../food-inventory')}>
          <span className="text-right ml-auto mr-4 text-maroon hover:text-hot-pink">View All</span>
        </button>
      </div>
        ...
        {items.slice(0, 10).map((val, key) => {
          return (
              <li key={key} className="flex bg-baby-powder justify-between m-2 rounded-lg">
                  <span className="px-4 py-2 text-left text-maroon">{val.name}</span>
                  <span className="px-10 py-2 text-right ml-auto text-maroon">{val.expiry}</span>
